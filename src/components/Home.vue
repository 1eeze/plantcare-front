<template>
  <div class="home">
    ...
    <div class="header">
      ...
      <div class="location">
        <p class="greeting">안녕하세요, {{ nickname }} 님!</p>
...
import { supabase } from '@/utils/supabase.js' // 2. supabase import 추가
...
onMounted(async () => {
  try {
    ...
    if (!user) return // 사용자가 없으면 '사용자' 님으로 유지

    // Users 테이블에서 닉네임(name) 가져오기
    ...
    console.error('홈페이지 닉네임 로드 에러:', error.message)
  }
})
